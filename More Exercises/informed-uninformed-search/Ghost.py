...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        in_table = state[0] >= 0 and state[0] < n and state[1] >= 0 and state[1] < n
        return in_table and state not in walls
    ...
    def successor(self, state):
        ...
        for i in range(3):
            ...
            # Gore
            if self.check_valid((ghost[0], ghost[1] + j), self.walls, self.n):
                successors[f'Gore {j}'] = tuple((ghost[0], ghost[1] + j))

            # Desno
            if self.check_valid((ghost[0] + j, ghost[1]), self.walls, self.n):
                successors[f'Desno {j}'] = tuple((ghost[0] + j, ghost[1]))
    ...
    def h(self, node):
        ghost = list(node.state)
        goal_p = self.goal

        sum = self.mhd(ghost[0], ghost[1], goal_p[0], goal_p[1])
