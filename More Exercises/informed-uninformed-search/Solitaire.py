...
class Soliter(Problem):
    def __init__(self, n, opponents, initial, goal=None):
        super().__init__(initial, goal)
        self.n = n
        self.opponents = opponents

    def check_valid(self, x, y, n):
        in_table = x >= 0 and x < n and y >= 0 and y < n
        return in_table and (x, y) not in self.opponents
    ...
    def successor(self, state):
        ...
        for i in range(len(balls)):
            ...
            # gore-levo
            if self.check_valid(ball[0] - 2, ball[1] + 2, self.n) and (ball[0] - 2, ball[1] + 2) not in balls:
                if (ball[0] - 1, ball[0] + 1) in balls:
                    new_balls = [b for b in balls if b != ball and b != (ball[0] - 1, ball[1] + 1)]
                    new_balls.append((ball[0] - 2, ball[1] + 2))
                    succ[f'Gore-levo: (x={ball[0]},y={ball[1]})'] = tuple(new_balls)

            # gore-desno
            if self.check_valid(ball[0] + 2, ball[1] + 2, self.n) and (ball[0] + 2, ball[1] + 2) not in balls:
                if (ball[0] + 1, ball[1] + 1) in balls:
                    new_balls = [b for b in balls if b != ball and b != (ball[0] + 1, ball[1] + 1)]
                    new_balls.append((ball[0] + 2, ball[1] + 2))
                    succ[f'Gore-desno: (x={ball[0]},y={ball[1]})'] = tuple(new_balls)
            ...
            if self.check_valid(ball[0] - 2, ball[1] - 2, self.n) and (ball[0] - 2, ball[1] - 2) not in balls:
                if (ball[0] - 1, ball[1] - 1) in balls:
                    new_balls = [b for b in balls if b != ball and b != (ball[0] - 1, ball[1] - 1)]
                    new_balls.append((ball[0] - 2, ball[1] - 2))
                    succ[f'Dolu-levo: (x={ball[0]},y={ball[1]})'] = tuple(new_balls)
            ...
            if self.check_valid(ball[0] + 2, ball[1] - 2, self.n) and (ball[0] + 2, ball[1] - 2) not in balls:
                if (ball[0] + 1, ball[1] - 1) in balls:
                    new_balls = [b for b in balls if b != ball and b != (ball[0] + 1, ball[1] - 1)]
                    new_balls.append((ball[0] + 2, ball[1] - 2))
                    succ[f'Dolu-desno: (x={ball[0]},y={ball[1]})'] = tuple(new_balls)

            # levo
            if self.check_valid(ball[0] - 2, ball[1], self.n) and (ball[0] - 2, ball[1]) not in balls:
                if (ball[0] - 1, ball[1]) in balls:
                    new_balls = [b for b in balls if b != ball and b != (ball[0] - 1, ball[1])]
                    new_balls.append((ball[0] - 2, ball[1]))
                    succ[f'Levo: (x={ball[0]},y={ball[1]})'] = tuple(new_balls)
            ...
            if self.check_valid(ball[0] + 2, ball[1], self.n) and (ball[0] + 2, ball[1]) not in balls:
                if (ball[0] + 1, ball[1]) in balls:
                    new_balls = [b for b in balls if b != ball and b != (ball[0] + 1, ball[1])]
                    new_balls.append((ball[0] + 2, ball[1]))
                    succ[f'Desno: (x={ball[0]},y={ball[1]})'] = tuple(new_balls)
    ...
    def goal_test(self, state):
        points = list(state)
        grey_field = math.ceil(self.n / 2) - 1
        if len(points) == 1:
            if (points[0][0], points[0][1]) == (grey_field, self.n - 1):
                return True
        else:
            return False
...
if __name__ == "__main__":
    ...
    solitaire = Soliter(N, obstacles, points)
