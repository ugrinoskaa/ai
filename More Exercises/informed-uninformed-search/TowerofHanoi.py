...
class Kula(Problem):
    ...
    def successor(self, state):
        successor = dict()
        # state = ( (3,2,1), (), () )
        # MOVE TOP BLOCK FROM PILLAR i TO PILLAR j  --AKCII
        number_pillars = len(state)
        pillars = list(state)
        for i in range(number_pillars):
            for j in range(number_pillars):
                if i != j:
                    i_pillar = list(pillars[i])  # momentalen
                    j_pillar = list(pillars[j])  # sleden
                    if len(i_pillar) == 0:
                        continue
                    if len(j_pillar) == 0 or i_pillar[-1] <= j_pillar[-1]:
                        new_i_pillar = i_pillar.copy()[:-1]
                        new_j_pillar = j_pillar.copy()
                        new_j_pillar.append(i_pillar[-1])
                        new_pillars = pillars.copy()
                        new_pillars[i] = tuple(new_i_pillar)
                        new_pillars[j] = tuple(new_j_pillar)
                        successor[f'MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}'] = tuple(new_pillars)
...
if __name__ == "__main__":
    ...
    kula = Kula(initial_towers, goal_towers)
    result = breadth_first_graph_search(kula).solution()
    actions = len(result)
    print(f'Number of action {actions}')
    print(result)
